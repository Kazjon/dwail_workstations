...
def test_setup_ray_head_starts_head_node(mocker):
    ...
    mock_run.assert_called_once_with(["/usr/bin/ray", "stop"], capture_output=True)
    mock_run_internal.assert_called_once_with(["/usr/bin/ray", "start", "--head", "--port=6379"])


def test_setup_ray_worker_connects_to_head(mocker):
    ...
    mock_run_internal.assert_called_once_with(
        ["/usr/bin/ray", "start", "--address=10.147.20.5:6379"]
    )


def test_setup_ray_defaults_to_head_when_neither_specified(mocker):
    mocker.patch("dwail_agent.install.subprocess.run")
    mocker.patch("dwail_agent.install.shutil.which", return_value="/usr/bin/ray")
    mock_run_internal = mocker.patch("dwail_agent.install._run")
